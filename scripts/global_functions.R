...
SegmentCells <- function (
  ...
  # Downscale image if xdim is provided
  if (is.numeric(xdim)) {
    if (verbose) {
      cat("Scaling down image... \n")
    }
    tmp.dir <- tempdir()
    fext <- tools::file_ext(impath)
    tmpim <- magick::image_read(impath) %>% magick::image_scale(paste0(xdim))
    outpath <- paste0(tmp.dir, "/downscaled_HE.", fext)
    magick::image_write(tmpim, path = outpath, format = fext)
    impath <- outpath
  }
